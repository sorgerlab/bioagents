...
class EKB(object):
    ...
    def get_term_ekb(self, term_id):
        ...
        if name_node:
            ...
            raise EkbError("Expected name for type %s." % node['type'])
        ...
        # Deal with any assoc-withs, if this is a SIGNALING-PATHWAY
        if node['type'].upper() == 'ONT::SIGNALING-PATHWAY':
            subject_id = self.graph.get_matching_node(term_id,
                                                      link='assoc-with')
            if subject_id:
                assoc_with = self.get_term_ekb(subject_id)
                term.append(assoc_with)

        return term

    def term_to_ekb(self, term_id):
        term = self.get_term_ekb(term_id)
...
class EkbError(Exception):
    def __init__(self, msg):
        full_msg = 'Error during generation of EKB: %s' % msg
        super(EkbError).__init__(self, full_msg)